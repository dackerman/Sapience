...
import { Article, ArticleWithSummary } from "@shared/schema";
import { Skeleton } from "@/components/ui/skeleton";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { CalendarIcon, BookOpenIcon, ThumbsUpIcon, StarIcon } from "lucide-react";
...
export default function ForYou() {
  ...
  useEffect(() => {
    if (recommendedArticles && recommendedArticles.length > 0 && !selectedArticle) {
  ...
  return (
    <div className="flex flex-col h-screen">
      ...
      <div className="flex flex-col md:flex-row flex-1 overflow-hidden">
        ...
        <div className={`md:hidden flex flex-col w-full ${selectedArticle ? 'hidden' : 'flex'} overflow-y-auto`}>
          <div className="p-4 border-b bg-gradient-to-r from-primary/90 to-primary/60 text-white">
            <h2 className="text-xl font-bold">For You</h2>
            <p className="text-sm opacity-90">Articles tailored to your interests</p>
          </div>
          ...
          {isLoading ? (
                ...
                >
                  ...
                  {article.recommendation && (
                    <div className="mt-2 text-xs text-slate-600 italic bg-slate-100 dark:bg-slate-800 dark:text-slate-300 p-2 rounded">
                      <span className="font-medium">Why we recommend this:</span> {article.recommendation.reasonForRecommendation}
                    </div>
                  )}
                </div>
              ))}
            ...
            <div className="flex flex-col items-center justify-center h-full p-6 text-center">
              ...
              <p className="text-sm text-slate-500 mt-2">
                We're still learning about your interests. Add more feeds or interact with more articles to get personalized recommendations.
              </p>
        ...
        <div className={`md:hidden w-full ${selectedArticle ? 'flex' : 'hidden'} flex-col h-full`}>
        ...
        {/* Desktop Layout */}
        <div className={`hidden md:flex border-r ${sidebarOpen ? 'w-96' : 'w-0'} transition-all duration-300 overflow-y-auto bg-slate-50 dark:bg-slate-900 flex-col`}>
          <div className="p-4 border-b bg-gradient-to-r from-primary/90 to-primary/60 text-white">
            <h2 className="text-xl font-bold">For You</h2>
            <p className="text-sm opacity-90">Articles tailored to your interests</p>
          </div>
